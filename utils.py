...
def create_data_lists(path, output_folder='./'):
    ...
    train_objects = []
    for abs_path in train_anno:
        objects = parse_annotation(abs_path)
    ...
    test_objects = []
    for abs_path in test_anno:
        objects = parse_annotation(abs_path)
        test_objects.append(objects)
        n_objects += len(objects)
    assert len(test_images) == len(test_anno)
...
def decimate(tensor, m):
    """ Уменьшает размерность тензора. Пример, если m = [2, 5, 10]:
    Тензор размерности (10, 15, 60) --> тензор размерностью (5, 3, 6) """
